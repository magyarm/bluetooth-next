...
static inline int
rdev_ed_scan_req(struct cfg802154_registered_device *rdev, struct wpan_dev *wpan_dev, u8 page, u32 scan_channels, u8 *level, size_t nlevel, u8 duration )
{
	int ret = 0;

	ret = rdev->ops->ed_scan_req(&rdev->wpan_phy, wpan_dev, page, scan_channels, level, nlevel, duration );
